...
The one thing to be aware of is that it doesn't support both 1 and 2 simultaneously.
It will effectively be set based on the initial connection.
PipeWire's https://docs.pipewire.org/page_module_raop_discover.html[RAOP Discover module] can be used to automatically discover and stream directly to the Piceiver, using AirPlay 1.
...
.Todo
* Run two instances of Shairport-Sync simultaneously, one for AirPlay 1 and one for AirPlay 2.
